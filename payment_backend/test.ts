...
let active_users: Collection<ActiveUser>;
...
async function connectDB() {
  try {
    ...
    customersCollection = db.collection<Customer>("customers");
    transactionsCollection = db.collection<Transaction>("transactions");
    active_users = db.collection<ActiveUser>("active");
...
async function createOrUpdateActiveUser(
  ...
  try {
    ...
    if (error.code === 11000) {
      const updateData: Partial<ActiveUser> = {
        paid_on: userData.paid_on,
        expires_on: userData.expires_on,
        service: userData.service,
      };
      ...
      const updatedUser = await active_users.findOne({
        email: userData.email!,
      });
      console.log(`Active user updated: ${userData.email}`);
      return updatedUser as ActiveUser;
    }
    throw error;
  }
}
